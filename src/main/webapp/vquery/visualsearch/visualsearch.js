...
// The search box is responsible for managing the many facet views and input views.
    VS.ui.SearchBox = Backbone.View.extend({
        ...
        addFacet: function(category, initialQuery, position, label, ext) {
            ...
            if(initialQuery.length==0 && ext && ext.type=="data")
            {
                initialQuery="?";
            }
            
            name=this.app.getName(category, initialQuery);
            ...
            if(initialQuery=="?")
            {
                var _this=this;
                setTimeout(function(){
                    var view = _this.app.searchBox.inputViews[position+1];
                    view.moveAutocomplete();
                    view.enableEdit();
                    //_this.app.searchBox.searchEvent({});
                    var query = _this.app.searchBox.value();
                    _this.app.options.callbacks.search(query, _this.app.searchQuery);
                },100);
                
                
//                model.set({value:"?"});
//                setTimeout(function(){jQuery.event.trigger({ type : 'keydown', which : 9 });},100);
                
//                this.app.searchBox.renderFacets();
//                this.app.searchBox.focusNextFacet(this.app.searchBox.facetViews[visualSearch.searchBox.facetViews.length-1], 1, {
//                    kipToFacet: true,
//                    selectText: true
//                });
            }         
        ...
        setupAutocomplete: function() {
            this.box.autocomplete({
                ...
                select: _.bind(function(e, ui) {
                    ...
                    this.set(ui.item.value,ui.item.label,ui.item.ext,true);
                    if("cls"!=this.model.get("category"))
                    {
                        var _this=this;
                        _this.app.searchBox.addFacet("cls",ui.item.value,_this.app.searchQuery.models.length,ui.item.label,ui.item.ext);
                        setTimeout(function(){                            
//                            var view = _this.app.searchBox.inputViews[_this.app.searchBox.inputViews.length - 1];
//                            view.moveAutocomplete();
//                            view.enableEdit();
                            _this.app.searchBox.searchEvent(e);
                        },100);
                        this.app.searchBox.renderFacets();
                    }else
                    {
        ...
        set: function(value, valueLabel, valueExt, select) {
            if (!value)
                return;
            this.model.set({'value': value});
            this.model.set({'valueLabel': valueLabel});
            this.model.set({'valueExt': valueExt});
            this.model.set({'name': this.app.getName(this.model.get('category'),value,select)});            
        },
...
// There is one input placed in between all facets.
    VS.ui.SearchInput = Backbone.View.extend({
        ...
        setupAutocomplete: function() {
            this.box.autocomplete({
                ...
                select: _.bind(function(e, ui) {
                    ...
                    {
                        this.app.searchBox.addFacet(ui.item.value, '', position, ui.item.label, ui.item.ext);
                    }
                    return false;
                }, this)
            });
        ...
        addFocus: function() {
            this.app.options.position=this.options.position;
            this.flags.canClose = false;
            ...
            return this.each(function() {
                ...
                $input.unbind(events).bind(events, function(e, realEvent) {
                    ...
                    if (VS.app.hotkeys.key(e) == 'backspace') {
                        ...
                        if (position > 0)
                            value = value.slice(0, position - 1) +
